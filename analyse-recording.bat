...
for /f "tokens=*" %%i in ('docker images -q dpctf-dof:v3.0.0 2^>nul') do set image=%%i
...
docker run -it --rm --network "host" -v %dirname%:/usr/app/recordings -v %~dp0observation_logs:/usr/app/device-observation-framework/logs -v %~dp0observation_results:/usr/app/device-observation-framework/results -e RECORDING_FILENAME=%filename% %args% dpctf-dof:v3.0.0 %start_args%
